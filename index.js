const { MailListener } = require("mail-listener5");   // NOTE: A FUTURE VERSION (release date TBA) will not require ES6 destructuring or referring to the class after the require statement (i.e. require('mail-listener5').MailListener). At this stage, this is necessary because index.js exports the MailListener class as a property of module.exports.
...
const mailListener = new MailListener({
    ...
    searchFilter: [ 'UNSEEN', ['SINCE', 'Jul 08, 2022']], // the search filter being used after an IDLE notification has been retrieved
...
mailListener.on("mail", function(mail, seqno) {
    ...
    // if(to.map(x => x.address).some(x => /^rfq.*$/.test(x))) {
        // processReceivedEmail(mail);
    // }
})
